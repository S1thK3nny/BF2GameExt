...
### Soldier Systems
- **Prone Stance** - Re-enables, fixes, and adapts the cut prone posture system. Double-tap crouch to go prone, any crouch press to stand back up. Includes a terrain rotation fix that prevented prone from working on slopes. The prone animations live in their own `prone.lvl` (the `human_5` sub-bank), which is read automatically right after every `ingame.lvl` read. Drop `prone.lvl` in `data\_lvl_pc\`; if it isn't there, prone stays disabled for that mission. INI: `[Features] Prone=1`
...
A runtime build-dispatch layer resolves per-build addresses, so an increasing set of hook-based features now runs on Steam and GOG in addition to the binary patches — including prone, aim assist, animation bank appending, the first-person animation banks (and FP sprint) on Steam, the terrain/prop/sky fixes, the vehicle first/third-person toggle, the cloth (cape) collision fixes, and the shield channel fix. The full Lua API and the remaining hooks currently target BF2_modtools only; broader GoG/Steam coverage is ongoing.
