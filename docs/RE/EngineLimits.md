...
The content pipeline intends to split meshes into segments referencing at most
15 bones. The final output must be checked: the supplied PC ModelMunge can emit
an 18-bone segment from the original nine-chain Fisto source while reporting a
13/15 split and returning success. A boundary-mask counting error followed by
vertex copying grows the retained segment, which the splitter does not recheck.
There is no runtime clamp to protect against this malformed compiled asset.
...
wrong — that arithmetic applies to a single segment. Validating the final segment
palettes and all vertex/skin indices is necessary; a successful munger exit is
insufficient.

The real character ceiling is `ZephyrSkeleton<32>`. See below.

### Unchecked upload, including malformed munger output

The render path reads `numBoneMatrices` raw from `RedSegment::m_uiNumBones` with no
...
rejects the oversized `SetVertexShaderConstantF`**. This was reproduced with the
actual SetMatrix4x3 instructions: fifteen matrices stay within the array; eighteen
overwrite 144 bytes beyond it. The Fisto failure was generated by stock tools.

On the retained `BF2_modtools_NoDVD.exe`, the upload is in `0x0080E510`
(`0x0080E5D5..0x0080E63B`), with another path at `0x0080F4F0` and SetMatrix4x3
at `0x008054A0`. The previously cited `0x00882CA0` is a sound routine on that
binary and must not be used as its renderer address. The palette is `0x00AE2BC8`
and the array ends at `0x00AE2E98`. The verified executable's SHA-256 is
`672265381d982551038df287db7b2553f8f32c48c3001d864bf5a561ff4741c8`.
This content-pipeline finding does not add a renderer patch.
...
The renderer can handle more joints across valid segments; final palette
validation remains a separate requirement from increasing animator capacity.
