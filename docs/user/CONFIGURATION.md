...
| Key | Default | Description |
|-----|---------|-------------|
| `TrueFirstPersonBody` | `0` | SHELVED, KNOWN BROKEN - do not enable. Was an attempt to render your own body in first person so third person animation plays. Tested 2026-08-21: it puts the camera back in third person entirely, because the call it skips does more than decide visibility - it participates in driving the first person path. Kept only so the address work is not lost. Leave at 0 |
