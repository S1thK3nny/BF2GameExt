...
| `AIUpdateBudget` | `0` | How many AI may make a new decision each turn. 0 keeps the stock 10. A unit that misses its turn keeps walking and shooting but never picks a new goal, which is what standing around looks like. Raising this costs frame time and only helps if the budget is what is actually holding them back, so check with AIUpdateDiag before changing it. Range 10 to 127 |
