...
struct AimAssistAddrs {
    uintptr_t player_controller_update;   // PlayerController::Update
    uintptr_t apply_damage;               // Damageable::ApplyDamage (modtools=6-param, retail=3-param)
    uintptr_t lockon_mgr_array;           // LockOnManager* global array (indexed by playerIndex)
    uintptr_t get_cur_wpn;                // Controllable::GetCurWpn → Weapon*
    uintptr_t set_target_locked_obj;      // Controllable::SetTargetLockedObj(__thiscall, GameObject* target) RET 4
    uintptr_t get_vehicle_if_not_me;      // Controllable::GetEnteredRemoteOrVehicleIfNotMe(__thiscall) → uint (0 = on foot)
    uintptr_t m_camera_global;            // RedCamera* global (for screen-space projection)
    unsigned  wpn_class_vert_threshold;   // WeaponClass offset for vertical auto-aim size
    unsigned  wpn_class_horiz_threshold;  // WeaponClass offset for horizontal auto-aim size
    uintptr_t team_get_objects_in_range;  // TeamManager::sGetObjectsInRange (__cdecl, returns validated GameObjects)
};

static constexpr AimAssistAddrs MODTOOLS_ADDRS = {
   .player_controller_update = 0x0059B460,
   .apply_damage = 0x004CF900,
   .lockon_mgr_array = 0x00B30698,
   .get_cur_wpn = 0x00413782,
   .set_target_locked_obj = 0x00411013,   // thunk → 0x005e0d30
   .get_vehicle_if_not_me = 0x00416f40,   // thunk → 0x005de940
   .m_camera_global = 0x00d61ddc,
   .wpn_class_vert_threshold = 0x298,
   .wpn_class_horiz_threshold = 0x29C,
   .team_get_objects_in_range = 0x0048f210,
};

static constexpr AimAssistAddrs STEAM_ADDRS = {
   .player_controller_update = 0x0061A2B0,
   .apply_damage = 0x00489340,
   .lockon_mgr_array = 0x01E57400,
   .get_cur_wpn = 0x00484310,
   .set_target_locked_obj = 0x004844A0,
   .get_vehicle_if_not_me = 0x00483A10,
...
static constexpr AimAssistAddrs GOG_ADDRS = {
   .player_controller_update = 0x0061B320,
   .apply_damage = 0x00489340,
   .lockon_mgr_array = 0x01E588B0,
   .get_cur_wpn = 0x00484310,
   .set_target_locked_obj = 0x004844A0,
   .get_vehicle_if_not_me = 0x00483A10,
...
{
    uintptr_t addr = g_game.num_joysticks_global;
    if (!addr) return false;
    uintptr_t base = (uintptr_t)GetModuleHandleW(nullptr);
    return *(int*)(addr - 0x400000u + base) > 0;
}
...
// GetEnteredRemoteOrVehicleIfNotMe — returns 0 if on foot, nonzero if in vehicle/remote
// __thiscall(Controllable*), returns uint
using fn_GetVehicleIfNotMe = uint32_t(__thiscall*)(void* ctrl);
static fn_GetVehicleIfNotMe s_getVehicleIfNotMe = nullptr;
...
// Modtools / retail: using 6-param base (__thiscall, RET 0x14)
...
static void __fastcall hooked_PCUpdate(void* thisPtr, void* /*edx*/, float dt)
{
    ...
    // GetEnteredRemoteOrVehicleIfNotMe returns 0 when ctrl IS the vehicle
    // (the "IfNotMe" clause), which happens because PlayerController.mOwner
    // switches to the vehicle's Controllable when entering.
    ...
    // Only runs when aimAssistAllowed == true
    // =====================================================================
...
{
    if (!s_aimAssistEnabled) {
        dbg_log("[AimAssist] Disabled via INI\n");
        return;
    }

    const AimAssistAddrs* addrs = nullptr;
    switch (g_exeType) {
    ...
    default:
        dbg_log("[AimAssist] Skipping — unsupported build\n");
        return;
    }
    ...
    s_lockOnMgrArray = resolve(addrs->lockon_mgr_array);
    s_cameraGlobal = resolve(addrs->m_camera_global);
    s_getCurWpn = (fn_GetCurWpn)resolve(addrs->get_cur_wpn);
    ...
    // Auto-lock-on-hit
    if (s_autoLockOnHit) {
        original_ApplyDamage6 = (fn_ApplyDamage6)resolve(addrs->apply_damage);
    ...
    if (result == NO_ERROR) {
        dbg_log("[AimAssist] Hook installed (friction=%.1f, trackRate=%.1f, lockBreak=%.2fs, autoLock=%s, snap=%.1f, proxFriction=%s/%.2f)\n",
            s_frictionStrength, s_pullStrength, s_lockBreakTime,
            s_autoLockOnHit ? "on" : "off", s_snapStrength,
            s_proximityFriction ? "on" : "off", s_proxFrictionRadius);
    }
    else {
        dbg_log("[AimAssist] ERROR: Detours commit failed (%ld)\n", result);
...
void aim_assist_uninstall()
{
    ...
    s_lockBreakTimer = 0.0f;
    s_currentWpnIsMelee = false;
    dbg_log("[AimAssist] Hook removed\n");
}