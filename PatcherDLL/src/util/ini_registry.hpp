...
inline constexpr IniEntry g_ini_registry[] = {
   ...
   INI_ENTRY("LimitIncreases", "TentacleLimit",      "0", "Raise how many tentacles a unit can have from 4 to 9. Bones per tentacle stays capped at 5. Keeps the original offline and multiplayer timing, and fixes extra chains losing their pose when another unit is drawn"),
   INI_PATCH("LimitIncreases", "ComboAnimIncrease",   "0", "Raise combo animation names from 30 to 90, distinct animation banks from 16 to 64, bank/weapon maps from 30 to 90 and references from 256 to 768. Includes the storage needed to play the extra animations. Off by default; enable for content that needs these limits", "Combo Anims Increase"),
