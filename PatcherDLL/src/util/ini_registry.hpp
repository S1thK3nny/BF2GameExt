...
inline constexpr IniEntry g_ini_registry[] = {
   ...
   INI_ENTRY("LimitIncreases", "SoftwareVoices", "0", "EXPERIMENTAL, keep off. Widens the software mixer so VoiceLimit also raises voices when EAX is not in use. The mixer relocation works, but the engine reconnects voices on an audio-mode change without checking for the 'no free input' marker and writes outside the table, which crashes at the main menu. Left in place so the work is not lost, but do not enable it"),
   ...
   INI_ENTRY("Fixes", "BranchRegionDebug", "0", "Diagnostic. Logs every step of EntityPath branch-region resolution to the game log so a BranchRegion that will not resolve can be traced. Off by default; it is noisy and changes nothing"),
   INI_ENTRY("Fixes", "SoundDiagnostic", "0", "Diagnostic. Reports to the game log how many sound voices this machine actually gets, how many sounds are being dropped for want of one, and how often two suspected causes of the random loud burst under EAX are reached. Off by default; it hooks the audio path and changes nothing"),
