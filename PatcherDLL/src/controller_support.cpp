                             ...
                             ButtonBinding* outBindings, int outMax)
{
   ...
   while (token && count < outMax) {
      ...
      if (action == -2) {
         dbg_log("[Controller] WARNING: unknown action '%s' — skipping\n", token);
...
void controller_setup_bindings()
{
   ...
   if (!numJoysticksAddr) {
      dbg_log("[Controller] No num_joysticks address — skipping binding setup\n");
      return;
   }
   ...
   int numJoysticks = *(int*)res(numJoysticksAddr);
   if (numJoysticks <= 0) {
      dbg_log("[Controller] No joysticks detected — skipping binding setup\n");
      return;
   }

   dbg_log("[Controller] %d joystick(s) detected, setting up bindings...\n", numJoysticks);

   // Enable joystick input processing.
   uintptr_t joyConfigAddr = g_game.joystick_config_base;
   if (joyConfigAddr) {
      *(char*)(res(joyConfigAddr) + 0xF94) = 1;
      dbg_log("[Controller] Joystick input enabled (flag at 0x%08X + 0xF94)\n",
              (unsigned)res(joyConfigAddr));
   } else {
      dbg_log("[Controller] Missing joystick_config_base — joystick may not be enabled\n");
   }

   uintptr_t ctrlBaseAddr = g_game.controller_base_global;
   if (!ctrlBaseAddr) {
      dbg_log("[Controller] Missing controller_base_global — cannot set up bindings\n");
      return;
   }

   uintptr_t ctrlBase = res(ctrlBaseAddr) + 0x428;
   ...
   for (int mode = 0; mode < CONTROL_MODE_COUNT; mode++) {
      ...
      dbg_log_verbose("[Controller] Mode %d (%s): %d bindings\n",
                      mode, section, count);
   }
   ...
   if (joyConfigAddr) {
      uintptr_t uiBase = res(joyConfigAddr) + 0x4A;
      for (int mode = 0; mode < CONTROL_MODE_COUNT; mode++)
         writeModeBindings(uiBase, 0x102, mode);
      dbg_log("[Controller] Wrote bindings to UI config\n");
   }

   // Step 2: Call the joystick init chain (discover device + sync bindings)
   if (joyConfigAddr && g_game.joystick_discover && g_game.joystick_sync) {
      uintptr_t configBase = res(joyConfigAddr);
      auto discover = (fn_joystick_discover)res(g_game.joystick_discover);
      auto sync     = (fn_joystick_sync)res(g_game.joystick_sync);
      discover(configBase, nullptr);
      sync(configBase, nullptr);
      dbg_log("[Controller] Called joystick discover + sync (device registered)\n");
   }
   ...
   // Dump runtime table for debugging — mode 0 only
   dbg_log_verbose("[Controller] 0x20BC table (mode 0) — buttons:\n");
   for (int a = 0; a < 18; a++) {
      uintptr_t e = rtTable + a * RT_ENTRY_SIZE;
      dbg_log_verbose("  action %2d: %02X %02X %02X %02X %02X %02X\n", a,
              *(unsigned char*)(e+0), *(unsigned char*)(e+1),
              *(unsigned char*)(e+2), *(unsigned char*)(e+3),
              *(unsigned char*)(e+4), *(unsigned char*)(e+5));
   }
   {
      uintptr_t e = rtTable + 29 * RT_ENTRY_SIZE;
      dbg_log_verbose("  action 29: %02X %02X %02X %02X %02X %02X  (roll)\n",
              *(unsigned char*)(e+0), *(unsigned char*)(e+1),
              *(unsigned char*)(e+2), *(unsigned char*)(e+3),
              *(unsigned char*)(e+4), *(unsigned char*)(e+5));
   }
   dbg_log_verbose("[Controller] 0x20BC table (mode 0) — axes:\n");
   for (int a = 30; a <= 33; a++) {
      uintptr_t e = rtTable + a * RT_ENTRY_SIZE;
      dbg_log_verbose("  action %2d: %02X %02X %02X %02X %02X %02X\n", a,
              *(unsigned char*)(e+0), *(unsigned char*)(e+1),
   ...
   dbg_log("[Controller] Bindings applied for %d modes\n", CONTROL_MODE_COUNT);
}
