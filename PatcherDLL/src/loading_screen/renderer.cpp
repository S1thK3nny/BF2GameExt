...
void __fastcall hooked_render_screen(void* ecx, void* edx)
{
    ...
    // RemoveToolTips: hide the tips group element (ecx+0x970) so the original
    // RenderScreen skips the tips box and text. Visibility bit is at element+0x14.
    if (g_loadScreenCfg.removeToolTips && ecx) {
        *(uint32_t*)((uint8_t*)ecx + 0x970 + 0x14) &= ~0x100u;
    }

    // RemoveLoadingBar: zero all LED alpha values so they render invisible,
    // and hide the bar decoration elements at known offsets.
    if (g_loadScreenCfg.removeLoadingBar && ecx) {
        // Zero LED alphas — ProgressIndicator+0x94 points to the alpha float array
        int numLEDs = *(int*)((uint8_t*)ecx + 0xddc);  // PI+0xac = LED count
        float* alphas = *(float**)((uint8_t*)ecx + 0xdc4); // PI+0x94 = alpha array
        if (alphas && numLEDs > 0 && numLEDs < 200) {
            for (int i = 0; i < numLEDs; ++i)
                alphas[i] = 0.0f;
        }
        // Hide bar decoration elements (visibility bit at element+0x14)
        static constexpr uintptr_t kBarElements[] = { 0xe00, 0xea0, 0xf40, 0xfe0 };
        for (uintptr_t off : kBarElements)
            *(uint32_t*)((uint8_t*)ecx + off + 0x14) &= ~0x100u;
    }
